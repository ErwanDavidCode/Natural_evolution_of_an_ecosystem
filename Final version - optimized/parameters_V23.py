...
factor_recording_video = 4 # On enregesi tre une image tous les "factor_recording_video" pas de temps. Plus factor_recording_video est grand, moins on enregistre d'images. 1 pour enregistrer toutes les images
fps = 30 / factor_recording_video #fps de la vidéo
video_scale = 2 # ATTENTION: prendre qqc qui divise la taille de la carte. Explication: 1 = full res, 2 = 2x plus petit (1200 -> 600), 3 = 400 etc.

time_to_shuffle = 1000 #temps pour mélanger la liste des individus. Plus il est petit, plus on mélange souvent la liste des individus


#initialisation - big simu -------------------------------------------------------------------------------
# size_modification = 2 #Divise par 2 la taille de tout : revient a faire un zoom out. J'ai mis ca car limite de carte 4000*4000 avec open cv. Le size_multiplication permet de simuler des cartes plus grandes. #SIZE MODIFCATION
# taille_carte = 4000
# max_individu = 100000
# nbr_individus_init = 400
# max_plantes = 100000
...
seed_attach_prob = 0.1      # proba par tick d'attraper des graines si contact plante
seed_packet_energy = 2.0     # énergie "embarquée" dans un paquet de graines (prélevée sur la plante)
seed_drop_prob = 0.05        # proba par tick de déposer une plantule
seed_energy = 5.0            # énergie d'une plantule
seed_bank_max = 20.0  # énergie max stockée sous forme de graines
...
#bruit
ecoute_rayon_init = 400/size_modification #parametre physique qui dépend du milieu on va dire #SIZE MODIFCATION
...
eatable_draw_min = 0.6     # rayon min affiché (évite les points invisibles)


# Couleurs graphe + simulation (BGR). Pour les eatables: [jeune, vieux]
colors = {
    0: [(0, 255, 0),   (0, 60, 0)],       # plante: vert vif -> vert foncé
    1: [(60, 140, 210),(20, 50, 80)],     # troph: marron "pétant" -> marron foncé
    2: [(0, 0, 255),   (0, 0, 70)],       # meat: rouge vif -> rouge foncé

    3: (255, 0, 0),    # individu (inchangé)
    4: (0, 110, 24),
    5: (0, 180, 240),
    6: (142, 142, 0),
    7: (0, 0, 0),
...
nbr_modifications_body = 3 #less physical mutations than brain mutations for the brain to have the time to adapt to the new body
...
liste_entrees_supplementaires_init = {"energie" : 1} #doit etre complementaire a liste_entrees_supplementaires_possibles_init
liste_entrees_supplementaires_par_part_init = {}
liste_sorties_supplementaires_init = {"attaque" : 1} #doit etre complementaire a liste_sorties_supplementaires_possibles_init
